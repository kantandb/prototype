...
func TestPathCursorRoundTrip(t *testing.T) {
	...
	cursor := pathCursor{
		...
		database: "users", pathHash: pathHash(`$["age"]`), index: "age",
	...
	if got.method != cursor.method || got.dialect != cursor.dialect || got.order != cursor.order || got.database != cursor.database || got.pathHash != cursor.pathHash || got.index != cursor.index || got.op != cursor.op || !bytes.Equal(got.value, cursor.value) || !bytes.Equal(got.lastValue, cursor.lastValue) || got.id != cursor.id {
...
func TestPathCursorBinding(t *testing.T) {
	...
	cursor := pathCursor{
		...
		database: "users", pathHash: pathHash(`$["age"]`), op: cmpGE, value: value, id: testCursorID,
	}
	token, err := encodePathCursor(box, cursor)
	if err != nil {
		t.Fatalf("encodePathCursor() error = %v", err)
	...
	tests := []struct {
		...
		{name: "order", database: "users", plan: pathPlan{text: plan.text, index: "age"}, query: query, value: value},
